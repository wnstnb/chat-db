...
// Database schema for reference (for generating SQL queries)
export const SCHEMA = `
Table: pages(
...
// Example queries for the model to reference
...
// Helper function to clean up SQL query
export function cleanupSqlQuery(rawQuery: string): string {
  // Remove triple backticks or any code fences
  let cleaned = rawQuery.replace(/```(?:sql)?/g, '');
  cleaned = cleaned.replace(/```/g, '');
  return cleaned.trim();
}

// Function to convert natural language to SQL
export async function convertToSql(nlQuery: string): Promise<string> {
  const prompt = `You are an expert data scientist specialized in SQL query generation. Analyze the provided PostgreSQL database schema and think step-by-step to produce precise and optimized SQL queries.
...
Your answer must be a single, valid PostgreSQL SQL query with no additional commentary.`;
  ...
  try {
    ...
    const cleanedQuery = cleanupSqlQuery(sqlQuery);
    
    // Validate that the query starts with an allowed SQL command
    const allowedPrefixes = [
      'select', 'with', 'insert', 'update', 'delete', 
      'create', 'drop', 'alter', 'desc', 'show', 'pragma'
    ];
    
    if (!allowedPrefixes.some(prefix => cleanedQuery.toLowerCase().startsWith(prefix))) {
      throw new Error('Generated query does not start with a valid SQL command. Aborting for safety.');
    }
...
// Function to determine if a SQL query is a read-only operation
export function isReadOnlyQuery(query: string): boolean {
  const lowerQuery = query.toLowerCase().trim();
  return lowerQuery.startsWith('select') || 
         (lowerQuery.startsWith('with') && !lowerQuery.includes('insert') && 
